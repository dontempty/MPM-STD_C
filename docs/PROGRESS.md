...
| **10⁶** | **256×128×96** | **9.96** | **11.0** | ✓ **~9.5%**, 전단 jet 복원, div~3e-14 |
| 10⁷ | — | **→ GPU** | 21.0 | CPU 불가(CFL dt 붕괴), P5 GPU 검증 |

**스케일링 검증**: 5.61→9.96 = decade당 1.77× → 지수 **~0.25 vs 논문 0.28** (정합). DHVC 물리 + Re_δ*~Ra^0.28 멱법칙 CPU 검증 완료(1e5 3%, 1e6 9.5%; 잔차는 중간 stream/span). 1e7+는 GPU(P5).
