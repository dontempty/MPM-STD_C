...
PressureSolverBase::PressureSolverBase(const grid::Grid&                  grid,
  ...
  : grid_(grid), sub_(sub), fields_(fields),
    problem_(problem), tdma_(tdma), bc_(bc) {
  ...
  for (int ii = 0; ii < n1_int_; ++ii)
    for (int jj = 0; jj < n2_int_; ++jj)
      for (int kk = 0; kk < n3_int_; ++kk) {
        ...
        P.host_at(ii + h, jj + h, kk + h) = val;
      }

  P.exchange_halo();
  bc_.apply_ghost(P, problem_.P);
}
...
void PressureSolverBase::project_(real_t dt,
  ...
  bc_.apply_ghost(U, problem_.U);
  bc_.apply_ghost(V, problem_.V);
  bc_.apply_ghost(W, problem_.W);
}
